...
create_subvols() {
    ...
    # Preserve existing user home directories in the dedicated @home subvolume.
    # Without this, /home becomes empty after reboot because @home is mounted over /home.
    if [ -d @/home ] && [ -n "$(ls -A @/home 2>/dev/null)" ]; then
        echo "--- Migrating /home content to @home ---"
        cp -a @/home/. @home/
    fi

...
unmount_everything() {
    ...
    umount $mp/boot
    umount $mp
    # Encryption disabled: no LUKS mapper is opened in this version.
    # if [ $only_subvols = no ]
    # then
...
show_help() {
    ...
    echo "Encryption is disabled in this version."
    echo
    echo "Usage: btrfs-encrypt-root [options] {root-dev} {boot-dev} [{efi-dev}]"
    echo
    echo "The devices must be given without the /dev/ prefix."
    echo
    echo "  --only-subvols            kept for compatibility; this version always only creates @ and @home"
    echo "  --enlarge                 ignored unless encryption code is re-enabled"
    echo
...
preparation
create_subvols "$1"
# Encryption disabled: this version only creates Btrfs subvolumes.
# if [ $only_subvols = no ]
# then
